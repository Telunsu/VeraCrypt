...
int init_logger(const char *log_dir, slog_level level)
{
	...
	int cur_pid = GetCurrentProcessId();
    ...
    snprintf(log_filepath, sizeof(log_filepath) - 1, "%s/%s_%d.log", log_dir, datestr, cur_pid);
...
void write_log(slog_level level, int print_stacktrace, const char *func_name, int line, const char* file_name, const char *fmt, ...)
{
    ...
    snprintf(log_line, sizeof(log_line) - 1, "%s %s %s %s:%d -| %s\n",
        level_str, timestr, file_name, func_name, line, log_content);
